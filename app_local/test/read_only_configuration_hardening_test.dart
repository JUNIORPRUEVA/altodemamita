...
void main() {
  ...
  test(
    'fetchByKeysWithDefaults no persiste claves faltantes al solo leer',
    () async {
  ...
  test(
    'loadSettings usa defaults en memoria sin sembrarlos en settings',
    () async {
      ...
      expect(settings.queueRetryInterval, const Duration(seconds: 3));
      expect(settings.realtimePollingInterval, const Duration(seconds: 2));
      expect(persistedRows, isEmpty);
    },
  );
  ...
  test(
    ...
    () async {
      ...
      expect(
        ...
        SyncConfigRepository.normalizeBackendBaseUrl(BASE_URL),
      );
    },
  );
}
