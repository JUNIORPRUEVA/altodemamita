...
void main() {
  test('fixed scenario probe output', () async {
    ...
    final installmentsHasDeletedAt = await _hasColumn(
      db,
      DatabaseSchema.installmentsTable,
      'deleted_at',
    );
    final paymentsHasDeletedAt = await _hasColumn(
      db,
    ...
    final createdInstallments = await db.query(
      DatabaseSchema.installmentsTable,
      where: installmentsHasDeletedAt
          ? 'venta_id = ? AND deleted_at IS NULL'
          : 'venta_id = ?',
      whereArgs: [saleId],
...
Future<bool> _hasColumn(dynamic db, String table, String column) async {
