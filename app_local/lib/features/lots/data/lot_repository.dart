...
class LotRepository {
  ...
  Future<Lot?> findByBlockAndLotNumber({
    ...
    // Buscar duplicados solo entre activos (deleted_at IS NULL)
    // usando manzana_numero AND solar_numero combinados
    final rows = await db.query(
      DatabaseSchema.lotsTable,
      where:
          'LOWER(TRIM(manzana_numero)) = LOWER(TRIM(?)) '
          'AND LOWER(TRIM(solar_numero)) = LOWER(TRIM(?)) '
          'AND deleted_at IS NULL',
      whereArgs: [normalizedBlock, normalizedLot],
      limit: 1,
    );
  ...
  Future<void> save(Lot lot) async {
    try {
      ...
      // Buscar duplicados incluyendo eliminados para logging
      final allRows = await db.query(
        DatabaseSchema.lotsTable,
        where:
            'LOWER(TRIM(manzana_numero)) = LOWER(TRIM(?)) '
            'AND LOWER(TRIM(solar_numero)) = LOWER(TRIM(?))',
        whereArgs: [normalizedBlock, normalizedLotNumber],
        limit: 2,
      );
      final foundActive = allRows.any(
        (r) =>
            r['deleted_at'] == null ||
            (r['deleted_at'] as String?)?.trim().isEmpty == true,
      );
      final foundDeleted = allRows.any(
        (r) =>
            r['deleted_at'] != null &&
            (r['deleted_at'] as String?)?.trim().isNotEmpty == true,
      );

      if (foundActive) {
        _log(
          '[DuplicateCheck][Lot] block=$normalizedBlock number=$normalizedLotNumber '
          'foundActive=true -> rejected',
        );
        ...
        _log(
          '[DuplicateCheck][Lot] block=$normalizedBlock number=$normalizedLotNumber '
          'foundDeleted=true foundActive=false -> allowed',
        );
      } else {
        _log(
          '[DuplicateCheck][Lot] block=$normalizedBlock number=$normalizedLotNumber '
          'noDuplicates=true -> allowed',
        );
      }
