...
import 'initial_cloud_upload_service.dart';
...
class SyncManager extends ChangeNotifier {
  ...
  final InitialCloudUploadService _initialCloudUploadService;
  ...
  ///
  /// Sube todos los datos locales históricos al backend una sola vez.
  /// Si ya se completó anteriormente, no hace nada.
  /// Si falla, no marca como completado y reintentará al próximo inicio.
  Future<void> _runInitialCloudUpload() async {
    try {
      final settings = await _configRepository.loadSettings();
      if (!settings.isConfigured) {
        debugPrint(
          '[SyncManager] InitialCloudUpload skipped: backend not configured',
        );
        return;
      }
