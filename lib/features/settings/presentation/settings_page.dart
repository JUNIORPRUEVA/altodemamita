      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 16),
              child: _SyncConflictResolutionPanel(isAdmin: auth.isAdmin),
            ),
...
class _SyncConflictResolutionPanel extends StatefulWidget {
  const _SyncConflictResolutionPanel({required this.isAdmin});

  final bool isAdmin;

  @override
  State<_SyncConflictResolutionPanel> createState() =>
      _SyncConflictResolutionPanelState();
}

class _SyncConflictResolutionPanelState
    extends State<_SyncConflictResolutionPanel> {
  final SyncConflictService _conflictService = SyncConflictService();
  late Future<List<SyncConflictRecord>> _future;

  @override
  void initState() {
    super.initState();
    _future = _loadConflicts();
  }

  @override
  void didUpdateWidget(covariant _SyncConflictResolutionPanel oldWidget) {
  ...
  Future<List<SyncConflictRecord>> _loadConflicts() {
    return _conflictService.listOpenConflicts(scope: 'products');
  }
  ...
  Future<void> _applyAction(
    ...
    try {
      ...
      ScaffoldMessenger.maybeOf(context)?.showSnackBar(
        SnackBar(content: Text('No se pudo resolver ${conflict.recordSyncId}: $error')),
      );
    }
  }

  Future<void> _openResolverDialog(
    BuildContext context,
    ...
    await showDialog<void>(
      ...
      builder: (dialogContext) {
        return AlertDialog(
          title: const Text('Resolver conflictos de sync'),
          content: SizedBox(
            width: 760,
            child: SingleChildScrollView(
              child: Column(
                ...
                children: conflicts.map((conflict) {
                  final serverTime = conflict.serverTime == null
                      ? 'Sin server_time'
                      : _formatDateTime(conflict.serverTime!);
                  final recommendation = _recommendedAction(conflict);
                  return Container(
                    margin: const EdgeInsets.only(bottom: 12),
                    padding: const EdgeInsets.all(12),
                    decoration: BoxDecoration(
                      color: const Color(0xFFFFF9E6),
                      border: Border.all(color: const Color(0xFFFFE096)),
                      borderRadius: BorderRadius.circular(10),
                    ),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text(
                          'Server time: $serverTime · Recomendado: $recommendation',
                          style: const TextStyle(
                            fontSize: 11.5,
                              ...
                              onPressed: !widget.isAdmin
                                  ...
                                  : () async {
                                      ...
                                      await _applyAction(
                                        ...
                                        conflict,
                                        () => _conflictService
                                            .resolveUsingServerVersion(conflict),
                                        'Conflicto resuelto usando la version del servidor.',
                                      );
                                    },
                              child: const Text('Usar version servidor'),
                            ),
                            OutlinedButton(
                              onPressed: !widget.isAdmin
                                  ? null
                                  : () async {
                                      ...
                                      await _applyAction(
                                        ...
                                        conflict,
                                        () => _conflictService
                                            .retryKeepLocalOverwrite(conflict),
                                        'Se reenviara la version local manualmente.',
                                      );
                                    },
                              child: const Text(
                                'Mantener local y sobrescribir manualmente',
                              ),
                            ),
                            TextButton(
                              onPressed: !widget.isAdmin
                                  ...
                                  : () async {
                                      ...
                                      await _applyAction(
                                        ...
                                        conflict,
                                        () => _conflictService
                                            .ignoreLocalConflict(conflict),
                                        'Conflicto marcado para ignorar la version local.',
                                      );
                                    },
                              child: const Text('Ignorar local'),
                            ),
                          ],
                        ),
                      ],
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    return FutureBuilder<List<SyncConflictRecord>>(
      future: _future,
      builder: (context, snapshot) {
              ...
              children: [
                Row(
                  children: [
                    ...
                    if (hasConflicts)
                      FilledButton.tonal(
                        onPressed: () => _openResolverDialog(context, conflicts),
                        child: const Text('Resolver conflictos de sync'),
                      ),
                  ],
                ),
                const SizedBox(height: 8),
                Text(
                  hasConflicts
                      ? '$count conflicto${count == 1 ? '' : 's'} de sincronizacion en products. La cola sigue funcionando para los demas registros.'
                      : 'Sin conflictos abiertos en products.',
                  style: const TextStyle(fontSize: 12.5),
                ),
                if (!widget.isAdmin) ...[
                  const SizedBox(height: 6),
                  const Text(
                    'Solo administradores pueden resolver conflictos manualmente.',
                    style: TextStyle(fontSize: 11.5, color: Color(0xFF6B7494)),
                  ),
                ],
              ],
            ),
  ...
  String _recommendedAction(SyncConflictRecord conflict) {
    final localVersion = conflict.localVersion ?? 0;
    final serverVersion = conflict.serverVersion ?? 0;
    if (serverVersion >= localVersion) {
      return 'Usar version servidor';
    }
    return 'Mantener local y sobrescribir manualmente';
  }
