...
class UserRolesSyncRepository implements SyncRepository {
  ...
  Future<void> mergeRemoteRecords(List<Map<String, dynamic>> records) async {
    ...
    await db.transaction((txn) async {
      for (final record in records) {
        ...
        if (existing.isEmpty) {
          await txn.insert(DatabaseSchema.userRolesTable, values);
        } else {
          await txn.update(
            DatabaseSchema.userRolesTable,
            values,
  ...
  Future<void> _markRows(Iterable<String> syncIds, String status) async {
    ...
    await db.rawUpdate(
      'UPDATE ${DatabaseSchema.userRolesTable} SET sync_status = ? WHERE sync_id IN ($placeholders)',
      [status, ...ids],
    );
  }
}
